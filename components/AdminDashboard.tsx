...
export const AdminDashboard: React.FC<Props> = ({ jobs, logs, employees, attendance, vehicles = [], vehicleLogs = [], justifications = [], customPrompts = [], permissions = {}, onSaveJob, onSaveEmployee, onSaveJustification, onSaveAiPrompts, onSavePermissions, onUpdateLog, currentUserRole, settings, onSaveSettings, onSaveAttendance, onDeleteAttendance, onSaveVehicle, onDeleteVehicle }) => {
  ...
  // ==========================================
  // LOGICA CALCOLO ORE CON SNAP 15m/30m
  // ==========================================
  const calculateDailyStats = (empId: string, dateStr: string) => {
    ...
    const parseTimeStr = (t: string) => { if(!t) return 0; const [h, m] = t.split(':').map(Number); return h * 60 + m; };
    ...
    const firstInMins = dayAttendance[0]?.type === 'ENTRATA' ? getMinutes(dayAttendance[0].timestamp) : null;
    const lunchOutMins = dayAttendance.length >= 3 && dayAttendance[1]?.type === 'USCITA' ? getMinutes(dayAttendance[1].timestamp) : null;
    const lunchInMins = dayAttendance.length >= 3 && dayAttendance[2]?.type === 'ENTRATA' ? getMinutes(dayAttendance[2].timestamp) : null;
    const lastOutMins = dayAttendance.length >= 2 && dayAttendance[dayAttendance.length - 1]?.type === 'USCITA' ? getMinutes(dayAttendance[dayAttendance.length - 1].timestamp) : null;

    let standardMinutes = 0;
    let overtimeMinutes = 0;
    let isLate = false;

    // Helper per calcolare l'inizio effettivo pagato con snap 15m sui ritardi
    const getEffectiveStart = (realMins: number, schedMins: number) => {
        // Se entra in anticipo rispetto al contratto -> paga dal contratto
        if (realMins <= schedMins) return schedMins;
        // Se entra entro la tolleranza -> paga dal contratto
        if (realMins <= schedMins + tolerance) return schedMins;
        // Se entra FUORI TOLLERANZA -> scatto al blocco di 15m successivo
        const delay = realMins - schedMins;
        const snappedDelay = Math.ceil(delay / latenessSnap) * latenessSnap;
        return schedMins + snappedDelay;
    };

    // 1. BLOCCO MATTUTINO (Inizio M -> Fine M)
    if (firstInMins !== null) {
    ...
    // 2. BLOCCO POMERIDIANO (Inizio A -> Fine A)
    // Se c'è una timbrata di rientro pausa, usiamo quella.
    // Altrimenti, se non c'è pausa bollata (Turno Unico), usiamo l'entrata mattutina come riferimento per il pomeriggio.
    let afternoonStartMins = null;
    if (lunchInMins !== null) {
        afternoonStartMins = getEffectiveStart(lunchInMins, schStartA);
    } else if (firstInMins !== null && !lunchOutMins && lastOutMins !== null && lastOutMins > schStartA) {
        // Lavoro continuo: l'inizio pomeridiano è la fine della pausa teorica (schStartA)
        // a meno che l'utente non sia entrato già nel pomeriggio.
        afternoonStartMins = Math.max(getEffectiveStart(firstInMins, schStartM), schStartA);
    }

    if (afternoonStartMins !== null && lastOutMins !== null) {
        const end = Math.min(lastOutMins, schEndA);
        if (end > afternoonStartMins) standardMinutes += (end - afternoonStartMins);
        if (lunchInMins !== null && lunchInMins > schStartA + tolerance) isLate = true;
    }

    // 3. STRAORDINARI (Solo ultima uscita vs fine orario pomeridiano)
    // Snap 30m arrotondato per DIFETTO (come da prassi industriale)
    ...
    return { 
        standardHours, overtime: overtimeHours, isLate, justification, records: dayAttendance,
        ...
        isAnomaly: dateStr < new Date().toISOString().split('T')[0] && dayAttendance.length > 0 && dayAttendance.length % 2 !== 0,
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8 print:p-0 print:max-w-none bg-slate-50 min-h-screen">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 print:hidden">
        <div>
          <h1 className="text-3xl font-bold text-slate-800">Gestionale Alea Sistemi</h1>
          <p className="text-slate-500 italic text-xs font-black tracking-widest uppercase">Pannello Amministratore</p>
        </div>
      </div>
      ...
      <div className="min-h-[400px]">
        {activeTab === 'OVERVIEW' && (
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex justify-between items-center"><div><p className="text-slate-400 text-xs font-bold uppercase mb-1">Commesse</p><h3 className="text-2xl font-black text-slate-800">{sortedJobStats.length}</h3></div><Briefcase className="text-blue-500 bg-blue-50 p-2 rounded-lg" size={40} /></div>
                <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex justify-between items-center"><div><p className="text-slate-400 text-xs font-bold uppercase mb-1">Produzione</p><h3 className="text-2xl font-black text-green-600">€ {sortedJobStats.reduce((acc, j) => acc + j.budgetValue, 0).toLocaleString()}</h3></div><TrendingUp className="text-green-500 bg-green-50 p-2 rounded-lg" size={40} /></div>
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex justify-between items-center"><div><p className="text-slate-400 text-xs font-bold uppercase mb-1">Utile Stimato</p><h3 className="text-2xl font-black text-purple-600">€ {sortedJobStats.reduce((acc, j) => acc + j.profitMargin, 0).toLocaleString()}</h3></div><CheckCircle2 className="text-purple-500 bg-purple-50 p-2 rounded-lg" size={40} /></div>
            </div>
        )}
        ...
        {activeTab === 'JOBS' && (
             <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="p-4 border-b border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center bg-slate-50 gap-4">
                    <h3 className="font-black text-slate-800 uppercase text-lg">Report Commesse</h3>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-slate-200">
                        <thead className="bg-slate-50 font-bold text-[10px] text-slate-400 uppercase tracking-widest">
                            <tr>
                                <th onClick={() => requestSort('code')} className="px-6 py-3 text-left cursor-pointer">Commessa</th>
                                <th onClick={() => requestSort('clientName')} className="px-6 py-3 text-left cursor-pointer">Cliente</th>
                                <th className="px-6 py-3 text-center">Ore Totali</th>
        ...
        {activeTab === 'HR' && (
             <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl shadow-md border border-slate-200 overflow-hidden">
                    <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50"><h3 className="font-black text-slate-800 uppercase tracking-tighter">Report Presenze Mensili</h3><button onClick={handleExportSummary} className="bg-green-600 text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-green-700 transition">Esporta Paghe</button></div>
                    <table className="min-w-full divide-y divide-slate-200">
                        <thead className="bg-slate-100 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-center">Giorni</th>
                                <th className="px-6 py-3 text-center">Ore Ordinarie</th>
                                <th className="px-6 py-3 text-center text-orange-600">Straord. (30m)</th>
                        ...
                        <tbody className="bg-white divide-y divide-slate-200">
                            {payrollStats.map(stat => (
                                <tr key={stat.id} className="hover:bg-blue-50/30 transition-colors">
                                    <td className="px-6 py-4 font-bold text-slate-900">{stat.name}<div className="text-[9px] text-slate-400 font-medium uppercase">{stat.department}</div></td>
                                    <td className="px-6 py-4 text-center font-bold text-slate-600">{stat.daysWorked}</td>
                                    ...
                                    <td className="px-6 py-4 text-right"><button onClick={() => setSelectedEmpForDetail(stat.id)} className="text-blue-600 font-bold text-xs hover:underline uppercase tracking-widest">Dettaglio</button></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                    ...
                    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
                        <div className="bg-white rounded-3xl w-full max-w-6xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden border-t-[8px] border-blue-600">
                            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                                <div><h3 className="text-2xl font-black text-slate-800">{employees.find(e => e.id === selectedEmpForDetail)?.name}</h3><p className="text-slate-400 font-bold uppercase text-xs tracking-widest">Cartellino Mensile - {selectedMonth}</p></div>
                                ...
                                <table className="w-full text-sm border-collapse">
                                    ...
                                    <tbody>
                                        {Array.from({length: new Date(parseInt(selectedMonth.split('-')[0]), parseInt(selectedMonth.split('-')[1]), 0).getDate()}, (_, i) => i + 1).map(day => {
                                            ...
                                            return (
                                                <tr key={day} className={`border-b ${isWeekend ? 'bg-slate-50 text-slate-300' : 'hover:bg-blue-50/20'}`}>
                                                    <td className="p-3 border font-bold text-xs">{day} <span className="text-[9px] uppercase text-slate-400 font-normal">{new Date(dateStr).toLocaleDateString('it-IT', {weekday:'short'})}</span></td>
                                                    <td className="p-1 border text-center"><TimeInput className={`w-full text-center bg-transparent border-none text-xs ${stats.isLate ? 'text-red-600 font-black' : ''}`} value={stats.firstIn || ''} onChange={(v) => handleTimeChange(selectedEmpForDetail, dateStr, v, stats.firstInId, 'ENTRATA')}/></td>
        ...
        {activeTab === 'MANAGE' && (
            <div className="space-y-6">
                <div className="flex gap-4 mb-6">
                    <button onClick={() => setManageSubTab('JOBS')} className={`px-6 py-2 rounded-xl font-black uppercase tracking-widest text-[10px] transition ${manageSubTab === 'JOBS' ? 'bg-blue-600 text-white shadow-lg' : 'bg-white text-slate-500 border border-slate-200'}`}>Commesse</button>
                    <button onClick={() => setManageSubTab('EMPLOYEES')} className={`px-6 py-2 rounded-xl font-black uppercase tracking-widest text-[10px] transition ${manageSubTab === 'EMPLOYEES' ? 'bg-blue-600 text-white shadow-lg' : 'bg-white text-slate-500 border border-slate-200'}`}>Staff</button>
                </div>
                {manageSubTab === 'JOBS' && (
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                        <div className="flex justify-between items-center mb-6"><h2 className="text-xl font-black text-slate-800 uppercase">Anagrafica Commesse</h2><button onClick={() => setIsEditingJob({})} className="bg-blue-600 text-white px-4 py-2 rounded-lg font-bold hover:bg-blue-700 transition flex items-center gap-2 shadow-md"><Plus size={18} /> Nuova</button></div>
                        <table className="min-w-full divide-y divide-slate-200">
                ...
                {manageSubTab === 'EMPLOYEES' && (
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                        <div className="flex justify-between items-center mb-6"><h2 className="text-xl font-black text-slate-800 uppercase">Gestione Personale</h2><button onClick={() => setIsEditingEmp({})} className="bg-blue-600 text-white px-4 py-2 rounded-lg font-bold shadow-md hover:bg-blue-700 transition">Nuovo</button></div>
                        {isEditingEmp && (
                            <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
                                <div className="bg-white p-8 rounded-3xl w-full max-w-2xl shadow-2xl border-t-[8px] border-blue-600 max-h-[90vh] overflow-y-auto">
                                    ...
                                    <div className="mt-8 flex justify-end gap-3"><button onClick={() => setIsEditingEmp(null)} className="px-6 py-2 text-slate-400 font-bold uppercase text-xs">Annulla</button><button onClick={handleSaveEmpForm} className="px-8 py-3 bg-blue-600 text-white rounded-xl font-black uppercase text-xs shadow-lg">Salva Profilo</button></div>
                                </div>
                            </div>
                        )}
                        <table className="min-w-full divide-y divide-slate-200">
                             <thead className="bg-slate-50 text-[10px] font-bold text-slate-400 uppercase tracking-widest"><tr><th className="px-6 py-3 text-left">Nome</th><th className="px-6 py-3 text-center">Contratto</th><th className="px-6 py-3 text-center">Tolleranza</th><th className="px-6 py-3"></th></tr></thead>
        ...
        {activeTab === 'CONFIG' && isSystem && (
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                <h2 className="text-xl font-black text-slate-800 uppercase mb-6 flex items-center gap-2"><Settings className="text-slate-600"/> Arrotondamenti & Snap</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200">
                        <h3 className="font-black text-slate-700 uppercase text-xs mb-4">Snap Straordinari Uscita (min)</h3>
                        ...
                        <p className="text-[10px] text-slate-400 mt-2 font-medium">Blocchi di arrotondamento per difetto (es. 30, 60...)</p>
                    </div>
                    <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200">
                        <h3 className="font-black text-slate-700 uppercase text-xs mb-4">Snap Ritardi Entrata (min)</h3>
                        <div className="p-3 bg-white rounded-xl font-black text-blue-600 text-center text-lg border border-slate-200 shadow-inner tracking-widest">15 MINUTI (FISSO)</div>
                        <p className="text-[10px] text-slate-400 mt-2 font-medium">Arrotondamento per eccesso al quarto d'ora fuori tolleranza.</p>
                    </div>
                </div>
            </div>
        )}
