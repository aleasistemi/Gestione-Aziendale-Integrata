...
const AttendanceKiosk: React.FC<Props> = ({ employees, onRecord, onExit, nfcEnabled }) => {
  ...
  // Rilevamento altezza per ottimizzazione 7"
    ...
    <div className={`min-h-screen bg-slate-100 flex flex-col items-center justify-center ${isShortScreen ? 'p-2' : 'p-6'} relative no-select overflow-hidden`}>
      ...
      {!selectedEmp ? (
        <div className="w-full max-w-2xl flex flex-col items-center">
          ...
          <div className={`flex flex-col items-center relative ${isShortScreen ? 'py-2' : 'py-6'}`}>
              <input 
                  ref={inputRef} 
                  type="text" 
                  value={scanValue} 
              ...
              <div className={`${isShortScreen ? 'px-6 py-2 mb-4 text-xs' : 'px-10 py-4 mb-10 text-sm'} rounded-full border border-slate-200 bg-white font-bold uppercase tracking-widest text-slate-400 shadow-sm`}>
                  Avvicina il badge al lettore
              </div>
              
              <button 
                  onClick={() => setShowPinPad(true)} 
                  className={`flex items-center gap-3 text-slate-600 hover:text-white hover:bg-[#EC1D25] transition-all bg-white ${isShortScreen ? 'px-8 py-3 rounded-xl' : 'px-12 py-5 rounded-2xl'} border border-slate-200 font-black uppercase tracking-widest shadow-md z-20`}
              >
                  <KeyRound size={isShortScreen ? 18 : 24} /> Digita PIN
              </button>
          </div>
        </div>
      ) : (
        <div className={`bg-white ${isShortScreen ? 'p-6' : 'p-12'} rounded-3xl shadow-2xl w-full max-w-2xl border-t-[8px] border-[#EC1D25] animate-in fade-in zoom-in duration-300`}>
             <div className={`flex justify-between items-center ${isShortScreen ? 'mb-6 pb-3' : 'mb-12 pb-6'} border-b border-slate-100`}>
                 <div className="flex items-center gap-4">
                     ...
                     <div>
                         <h2 className={`${isShortScreen ? 'text-xl' : 'text-3xl'} font-black text-slate-900`}>{selectedEmp.name}</h2>
                         <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">{selectedEmp.role}</p>
                     </div>
                 </div>
                 <button onClick={() => setSelectedEmp(null)} className="px-4 py-2 text-xs font-black text-slate-400 hover:text-red-500 transition-colors uppercase tracking-widest">Annulla</button>
             </div>
      ...
      {/* PIN Pad Modals - optimized for height */}
      {showPinPad && (
          <div className="fixed inset-0 bg-slate-950/90 flex items-center justify-center z-[100] p-4">
              <div className={`bg-white ${isShortScreen ? 'p-4' : 'p-8'} rounded-3xl w-full max-w-sm text-center shadow-2xl`}>
                  <h3 className={`${isShortScreen ? 'text-xl mb-4' : 'text-2xl mb-8'} font-black text-slate-800`}>PIN Personale</h3>
      ...
      {showExitPinPad && (
          <div className="fixed inset-0 bg-slate-950/95 flex items-center justify-center z-[110] p-4">
              <div className={`bg-white ${isShortScreen ? 'p-4' : 'p-8'} rounded-3xl w-full max-w-sm text-center shadow-2xl`}>
                  <h3 className={`${isShortScreen ? 'text-xl mb-4' : 'text-2xl mb-8'} font-black text-slate-800`}>Uscita Amministratore</h3>
                  <div className={`${isShortScreen ? 'text-3xl py-3 mb-6' : 'text-5xl py-6 mb-10'} font-mono bg-slate-100 rounded-2xl tracking-[0.5em] text-slate-900 font-black`}>
